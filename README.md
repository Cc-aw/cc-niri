...
- Focus Wide is a persistent per-Column property for the current KWin session. Leaving a Wide column shows the destination's normal pair; returning to that column automatically isolates it at 72% again. An off-screen return uses three explicit visual states: the old 50% pair, the scrolled destination pair, then the isolated 72% window (for example `1|2 -> 2|3 -> 3@72%`). The Bridge does not request the final expansion until the destination pair has finished scrolling and briefly settled. Pressing `Meta+Z` on it restores 50% pairing. Presentation never permanently changes the normal scroll offset.
