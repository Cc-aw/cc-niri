...
- Focus Wide is a persistent per-Column property for the current KWin session. Leaving a Wide column shows the destination's normal pair; returning to that column automatically isolates it at 72% again. Pressing `Meta+Z` on it restores 50% pairing. Presentation never permanently changes the normal scroll offset.
