...
- Focus Wide is a persistent per-Column property for the current KWin session. Leaving a Wide column shows the destination's normal pair; returning to that column automatically isolates it at 72% again. An off-screen return first renders the real 50% scrolling movement, then expands to 72% through a guarded Bridge handoff, so it cannot skip directly to the final geometry. Pressing `Meta+Z` on it restores 50% pairing. Presentation never permanently changes the normal scroll offset.
